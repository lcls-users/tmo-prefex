...
def plot_histogram_with_peaks(energy_data, run_num, port, retardation,
    ...
    # Get maximum count value and print it
    max_count = counts.max()
    print(f"Maximum count value for run {run_num}, port {port}: {max_count}")

    # Normalize counts
    counts = counts / max_count

    # Adjust 'height' and 'prominence' if they are not None
    if height is not None and height > 1:
        print("Warning: 'height' parameter should be between 0 and 1 after normalization.")
    if prominence is not None and prominence > 1:
        print("Warning: 'prominence' parameter should be between 0 and 1 after normalization.")
    ...
    for i, (peak_energy, peak_count) in enumerate(zip(peak_energies, peak_counts)):
        ...
        ax.annotate(f'{peak_energy:.2f} eV',
                    xy=(peak_energy, peak_count),
                    xytext=(0, 0.05), textcoords='offset points',
...
def plot_histogram_with_peaks_and_fits(energy_data, run_num, port, retardation,
    ...
    fig, ax = plt.subplots(figsize=(10, 6))
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
    ax.bar(bin_centers, counts_normalized, width=bin_width, align='center', alpha=0.7, edgecolor='black')
    ...
    for i, (peak_energy, peak_count_norm) in enumerate(zip(peak_energies, peak_counts)):
        ...
        try:
            ...
            y_fit = counts[start:end]  # Use original counts for fitting

            # Initial guesses for A, mu, sigma
            A_guess = counts[peaks[i]]
            ...
            y_gauss = gaussian(x_gauss, *popt) / max_count  # Normalize for plotting
