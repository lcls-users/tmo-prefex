...
def plot_histogram_with_peaks(energy_data, run_num, port, retardation,
    ...
    for peak_energy, peak_count in zip(peak_energies, peak_counts):
        ax.annotate(f'{peak_energy:.2f} eV', xy=(peak_energy, peak_count),
                    xytext=(0, 5), textcoords='offset points', ha='center', va='bottom', fontsize=8, color='red')

    # Set plot titles and labels
    ax.set_title(f'Energy Histogram with Peaks - Run {run_num}, Port {port}, Retardation {retardation}')
