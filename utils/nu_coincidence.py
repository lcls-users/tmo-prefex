...
def main():
    ...
    hist = np.zeros((1<<9,1<<9),dtype=np.uint64)
    tofs = {}
    addresses = {}
    nedges = {}
    ...
    for name in filenames:
        with h5py.File(os.path.join(path,name),'r') as f:
            ...
            for i in range(len(addresses['port_000'])):
                n1 = nedges['port_000'][i]
                n2 = nedges['port_180'][i]
                a1 = addresses['port_000'][i]
                a2 = addresses['port_180'][i]
                for t1 in tofs['port_000'][a1:a1+n1]:
                    for t2 in tofs['port_180'][a2:a2+n2]:
                        ind1 = max(0,min(hist.shape[0]-1, int(t1-t0)>>1))
                        ind2 = max(0,min(hist.shape[1]-1, int(t2-t0)>>1))
                        hist[ind1,ind2] += 1


    plt.imshow(hist,clim=(0,5))
    plt.colorbar()
    plt.show()
